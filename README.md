...
- `lib/seo.ts`: shared canonical URL, metadata, Open Graph, Twitter card, and JSON-LD builders.
...
- `components/JsonLd.tsx`: safe JSON-LD script renderer for structured data.
- `app/robots.ts`: dynamic `robots.txt` rules.
- `app/sitemap.ts`: dynamic `sitemap.xml` entries for home, products, and sellers.

## SEO And Crawler Support

The app uses Next.js App Router metadata so search engines, social previews, and LLM crawlers can understand the marketplace pages without relying on client-only rendering.

Global SEO is defined in `app/layout.tsx`:

- Metadata base URL from `NEXT_PUBLIC_SITE_URL`, falling back to `http://localhost:3000`.
- Default title template and site description.
- Canonical URL configuration.
- Robots directives that allow indexing and large previews.
- Open Graph metadata for rich link previews.
- Twitter/X summary large image metadata.
- JSON-LD `WebSite` structured data.
- JSON-LD `Organization` structured data.

Page-level SEO is defined per App Router route:

- `app/page.tsx`: home page title, description, canonical URL, Open Graph, Twitter card, and breadcrumb JSON-LD.
- `app/product/[id]/page.tsx`: dynamic product metadata from product data, canonical URL, Open Graph image, product JSON-LD, offer JSON-LD, aggregate rating JSON-LD, and breadcrumb JSON-LD.
- `app/seller/[id]/page.tsx`: dynamic seller metadata, canonical URL, seller Open Graph metadata, `LocalBusiness` JSON-LD, aggregate rating JSON-LD, and breadcrumb JSON-LD.
- `app/post-product/page.tsx`: metadata for the product posting workflow with `noindex` robots because it is an authenticated action page.
- `app/create-seller/page.tsx`: metadata for seller onboarding with `noindex` robots because it is an authenticated action page.

Product detail pages are crawler-friendly by design:

- Product title is rendered as the page `h1`.
- Product description is visible in server-rendered HTML.
- Price, previous price, discount, rating count, location, category, and condition are visible in HTML.
- Seller name, location, profile link, and seller bio are visible in HTML.
- Product images use descriptive alt text.
- FAQ-style product handoff and review content is rendered on the page.
- Product actions remain client-side, but the important product and seller content is not hidden behind client-only state.

Seller pages expose marketplace-style local business information:

- Seller name is rendered as the page `h1`.
- Seller bio, location, average rating, rating count, and product list are visible in HTML.
- JSON-LD uses `LocalBusiness` so crawlers can understand seller profile pages as local marketplace entities.

Crawler files:

- `/robots.txt` is generated by `app/robots.ts`.
- `/sitemap.xml` is generated by `app/sitemap.ts`.
- The sitemap includes the home page plus dynamic product and seller detail pages when the API is available.
- API routes and authenticated creation pages are excluded from broad crawling.
- Known LLM crawlers such as GPTBot, ChatGPT-User, ClaudeBot, and PerplexityBot are allowed to crawl public pages while `/api/` remains blocked.
...
This makes canonical URLs, Open Graph URLs, JSON-LD URLs, `robots.txt`, and `sitemap.xml` point to the real public domain.
