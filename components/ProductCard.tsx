...
function ProductCard({ product, isFocused, isScrolling, hasScrolled, cardRef }: ProductCardProps) {
  const [imageSrc, setImageSrc] = useState(product.imageUrl || genericFallbackImage);
  ...
  useEffect(() => {
    if (isFocused && hasScrolled && !isScrolling && product.videoUrl && videoRef.current) {
    ...
    <article className="product-card" ref={cardRef} data-product-id={product.id}>
      <div className="product-image-wrap">
        <img
          ...
          onError={() => {
            if (!hasTriedFallback) {
              setImageSrc(`https://picsum.photos/seed/${encodeURIComponent(product.id)}/900/700`);
        ...
        {product.videoUrl ? (
          <>
            <video
              ...
              poster={imageSrc}
            />
            <div className={`video-preview-indicator ${isPlaying ? 'visible' : ''}`}>
              Preview active
            </div>
          </>
        ) : null}
        <span className="discount-pill product-badge">{product.discountPercentage}% off</span>
      </div>
      <div className="product-card-content">
        ...
        <h2>{product.title}</h2>
        <p className="muted-text">{product.location} · {product.sellerName}</p>
        <div className="product-rating-row">
          <RatingStars
            rating={product.productRatingCount > 0 ? product.productRatingAverage : 4.0}
            label={product.productRatingCount > 0 ? `${product.productRatingAverage.toFixed(1)} out of 5` : 'No reviews yet'}
          />
          <span className="rating-count">
            {product.productRatingCount > 0 ? `${product.productRatingCount} reviews` : 'No reviews yet'}
          </span>
        </div>
